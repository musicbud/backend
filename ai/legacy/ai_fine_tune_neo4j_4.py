...
async def get_recommendations(user_id, item_features_matrix_sparse, user_features_matrix_sparse):
    try:
        ...
        user_features_vector = np.zeros((1, num_features))
        user_features_vector[0, artist_indices + track_indices] = 1
        ...
        top_n = 10
        top_artist_recommendations = np.argsort(-np.array(predictions_artist))[:top_n]
        top_track_recommendations = np.argsort(-np.array(predictions_track))[:top_n]

        logger.debug(f"Artist Predictions: {predictions_artist}")
        logger.debug(f"Top Artist Recommendations: {top_artist_recommendations}")
        logger.debug(f"Track Predictions: {predictions_track}")
        logger.debug(f"Top Track Recommendations: {top_track_recommendations}")

        # Get item names for recommendations
        recommended_artists = [item_dict_artist_reversed.get(idx) for idx in top_artist_recommendations]
        recommended_tracks = [item_dict_track_reversed.get(idx) for idx in top_track_recommendations]
        return recommended_artists, recommended_tracks
    except Exception as e:
        logger.error(f"Error getting recommendations: {e.__class__.__name__} - {e}")
...
async def run():
    ...
    if interactions_matrix is not None:
        model = await fine_tune_model(LightFM(), interactions_matrix, dataset, item_features_matrix_sparse, user_features_matrix_sparse)
        if model:
            recommended_artists, recommended_tracks = await get_recommendations(
                '33a55ade4b204e84a6c4c681bd751479', item_features_matrix_sparse, user_features_matrix_sparse)
